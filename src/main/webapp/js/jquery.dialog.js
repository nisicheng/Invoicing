...
{	
	return lhgdialog({
		...
		init: function(){
			var $this = $(this);
			this.DOM.inner.find("table").removeClass("ui_dialog");//loading组件，在tips的基础上去掉多余的样式和元素，只显示一张图片
			this.DOM.dialog.closest('tr').find("td").removeClass("ui_l");
			this.DOM.dialog.closest('tr').find("td").removeClass("ui_r");
		}
	}).time(time || 1.5, function(){
		if( callback ) this.config.close = callback;
	});
...
// 进度条形式
lhgdialog.progress = function( content, time, icon, lock, callback )
{	
	return lhgdialog({
		id: 'progress',
		zIndex: 2,
		title: false,
		// icon: 'loading01.gif',
		content: content || '<div class="ui-loading"><img src="/css/base/dialog/icons/loading01.gif" class="ui_icon_bg"><div>',
		cancel: false,
		padding: "0",
		fixed: true,
		lock: !!lock,
